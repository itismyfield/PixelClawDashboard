...
router.get("/api/cron-jobs", (_req, res) => {
  ...
  try {
    ...
    const jobs = (data.jobs || []).map((j: Record<string, unknown>) => ({
      ...
      schedule: j.schedule,
      state: j.state,
      discordChannelId: j.discordChannelId ?? null,
      description_ko: j.description_ko ?? null,
    }));
    res.json({ jobs });
  } catch {
    res.json({ jobs: [] });
...
// ── Machine Status (mac-mini local, mac-book SSH ping) ──
router.get("/api/machine-status", (_req, res) => {
  const machines = [
    { name: "mac-mini", online: true, lastChecked: Date.now() }, // always local
  ];
...
// ── Activity Heatmap (from skill_usage_events) ──
...
router.get("/api/streaks", (_req, res) => {
  ...
  // Calculate streaks from daily_activity table
  const agents = db.prepare("SELECT id, name, alias, name_ko, avatar_emoji, stats_xp FROM agents").all() as Array<{
    id: string; name: string; alias: string | null; name_ko: string; avatar_emoji: string; stats_xp: number;
  }>;
...
router.get("/api/achievements", (req, res) => {
  ...
  // Auto-generate achievements based on agent stats
...
// ── GitHub Issues (for CookingHeart or any configured repo) ──
router.get("/api/github-issues", async (req, res) => {
  const repo = (req.query.repo as string) || "user/CookingHeart";
  const state = (req.query.state as string) || "open";
  const limit = Math.min(parseInt(req.query.limit as string) || 20, 50);
  try {
    const result = execSync(
      `gh issue list --repo "${repo}" --state "${state}" --limit ${limit} --json number,title,state,labels,assignees,createdAt,updatedAt 2>/dev/null`,
      { timeout: 10000, encoding: "utf-8" },
    );
    const issues = JSON.parse(result);
    res.json({ issues, repo });
  } catch {
    res.json({ issues: [], repo, error: "gh CLI unavailable or repo not accessible" });
  }
});
