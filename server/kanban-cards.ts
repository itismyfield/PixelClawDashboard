...
export function triggerCounterModelReview(db: DatabaseSync, cardId: string, opts?: { bypassDod?: boolean }): boolean {
  ...
  const reviewInstructions = [
    ...
    ``,
    `## 결과 회신 (필수)`,
    `리뷰가 끝나면 \`send-agent-message\` 스킬로 아래 형식의 메시지를 회신하세요.`,
    ``,
    `**문제 없을 때:**`,
    `\`[리뷰결과:${reviewDispatchId}] pass\``,
    ``,
    `**개선 필요 시:**`,
    `\`[리뷰결과:${reviewDispatchId}] improve\``,
    `- 발견사항 1`,
    `- 발견사항 2`,
    ``,
    `- 반드시 \`[리뷰결과:${reviewDispatchId}]\` 접두어를 포함하세요.`,
    `- **회신하지 않으면 리뷰 결과가 반영되지 않습니다.**`,
    ``,
    `---`,
    `회신: channel:${originalProvider === "codex" ? agent.discord_channel_id_codex ?? agent.discord_channel_id : (agent.discord_prefer_alt ? agent.discord_channel_id_alt : agent.discord_channel_id)} | 접두어: [리뷰결과:${reviewDispatchId}]`,
  ].join("\n");
