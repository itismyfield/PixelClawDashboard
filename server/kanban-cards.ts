...
export function triggerCounterModelReview(db: DatabaseSync, cardId: string, opts?: { bypassDod?: boolean }): boolean {
  ...
  const reviewInstructions = [
    ...
    `- 코드를 읽고 검토 항목만 추출하세요`,
    `- **send-agent-message로 별도 회신하지 마세요** — 결과는 아래 .result.json 파일로만 반환`,
    ``,
    `## 리뷰 대상`,
    ...
    ``,
    `## 결과 반환 (필수)`,
    `리뷰가 끝나면 아래 curl 명령으로 결과를 제출하세요:`,
    ``,
    `**문제 없을 때 (pass):**`,
    `\`\`\`bash`,
    `curl -s -X POST http://127.0.0.1:8791/api/review-result \\`,
    `  -H "Content-Type: application/json" \\`,
    `  -H "Authorization: Bearer $SESSION_AUTH_TOKEN" \\`,
    `  -d '{"dispatch_id":"${reviewDispatchId}","summary":"리뷰 완료 - 문제 없음","review_verdict":{"overall":"pass","items":[]}}'`,
    `\`\`\``,
    ``,
    `**개선 필요 시 (improve):**`,
    `\`\`\`bash`,
    ...
    `  -H "Content-Type: application/json" \\`,
    `  -H "Authorization: Bearer $SESSION_AUTH_TOKEN" \\`,
    `  -d '{"dispatch_id":"${reviewDispatchId}","summary":"리뷰 완료 - 개선 필요","review_verdict":{"overall":"improve","items":[{"id":"item-1","category":"improve","summary":"발견 사항 요약"}]}}'`,
    `\`\`\``,
    ``,
    `- items 배열에 발견 사항을 추가하세요 (id, category:"improve", summary 필수).`,
    `- **이 API를 호출하지 않으면 리뷰 결과가 반영되지 않습니다.**`,
  ].join("\n");
  ...
  const handoff = {
    ...
    to: card.assignee_agent_id,
    type: "review",
    title: `[Review R${round}${card.github_issue_number ? ` #${card.github_issue_number}` : ""}] ${card.title}`,
    ...
    context: {
      ...
      issue_url: card.github_issue_url ?? undefined,
...
function dispatchReviewDecision(
  ...
  const instructions = [
    ...
    `**send-agent-message로 별도 회신하지 마세요** — decisions API 호출로만 처리합니다.`,
    ``,
    `### 리뷰 항목`,
    itemsDesc || "(항목 없음)",
    ``,
  ...
  const handoff = {
    ...
    title: `[Decision R${review.round}${card.github_issue_number ? ` #${card.github_issue_number}` : ""}] ${card.title}`,
...
function createReworkDispatch(
  ...
  const handoff = {
    ...
    to: card.assignee_agent_id,
    type: "generic",
    title: `[Rework R${review.round}${card.github_issue_number ? ` #${card.github_issue_number}` : ""}] ${card.title}`,
    parent_dispatch_id: review.review_dispatch_id,
    context: {
